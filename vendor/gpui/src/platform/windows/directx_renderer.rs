...
struct DirectXResources {
    ...
    // Scene snapshot used by component-level backdrop materials.
    backdrop_texture: ID3D11Texture2D,
    backdrop_srv: [Option<ID3D11ShaderResourceView>; 1],
...
impl DirectXRenderer {
    ...
    pub(crate) fn draw(&mut self, scene: &Scene) -> Result<()> {
        ...
        for batch in scene.batches() {
            match batch {
                ...
                PrimitiveBatch::BackdropBlurs(backdrops) => self.draw_backdrop_blurs(backdrops),
    ...
    fn draw_backdrop_blurs(&mut self, backdrops: &[BackdropBlur]) -> Result<()> {
        for backdrop in backdrops {
            // A render target cannot be sampled while it is bound for output.
            // Copy the already-painted scene into a shader-readable texture,
            // then render the material back onto the swap-chain target.
            unsafe {
                self.devices.device_context.PSSetShaderResources(0, Some(&[None]));
                self.devices.device_context.VSSetShaderResources(0, Some(&[None]));
