...
**
** The renderer copies the already-painted swap-chain target to t0 immediately
** before this draw, so every sample below is from content underneath the glass
** surface in the same GPUI scene.
*/
...
BackdropBlurVertexOutput backdrop_blur_vertex(uint vertex_id: SV_VertexID, uint backdrop_id: SV_InstanceID) {
    float2 unit_vertex = float2(float(vertex_id & 1u), 0.5 * float(vertex_id & 2u));
    BackdropBlur backdrop = backdrop_blurs[backdrop_id];
...
float4 sample_backdrop_blur(float2 uv, float2 texel, float radius) {
    // A compact 9-tap separable Gaussian approximation. Linear filtering
    // between taps keeps the pass smooth while avoiding an additional target.
    float2 step = texel * max(radius, 1.0);
    float4 color = t_sprite.SampleLevel(s_sprite, uv, 0.0) * 0.227027f;
    float2 sx = float2(step.x, 0.0f);
    float2 sy = float2(0.0f, step.y);
    color += (t_sprite.SampleLevel(s_sprite, uv + sx * 1.384615f, 0.0)
        + t_sprite.SampleLevel(s_sprite, uv - sx * 1.384615f, 0.0)
        + t_sprite.SampleLevel(s_sprite, uv + sy * 1.384615f, 0.0)
        + t_sprite.SampleLevel(s_sprite, uv - sy * 1.384615f, 0.0)) * 0.158108f;
    color += (t_sprite.SampleLevel(s_sprite, uv + sx * 3.230769f, 0.0)
        + t_sprite.SampleLevel(s_sprite, uv - sx * 3.230769f, 0.0)
        + t_sprite.SampleLevel(s_sprite, uv + sy * 3.230769f, 0.0)
        + t_sprite.SampleLevel(s_sprite, uv - sy * 3.230769f, 0.0)) * 0.035135f;
    return color;
}
