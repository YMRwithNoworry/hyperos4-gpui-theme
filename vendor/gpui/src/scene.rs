...
impl<'a> Iterator for BatchIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let mut orders_and_kinds = [
            ...
            (
                self.backdrop_blurs_iter.peek().map(|s| s.order),
        ...
        match batch_kind {
            ...
            PrimitiveKind::BackdropBlur => {
                ...
                while self
                    .backdrop_blurs_iter
                    .next_if(|backdrop_blur| {
                        (backdrop_blur.order, batch_kind) < max_order_and_kind
                    })
                    .is_some()
                {
...
/// A component-level backdrop material. The renderer snapshots the already
/// painted scene, applies a blur and a UV displacement, then composites the
/// result with the supplied tint and Fresnel reflection parameters.
#[derive(Clone, Debug)]
#[repr(C)]
pub(crate) struct BackdropBlur {
    ...
    pub height_transition_width: f32,
